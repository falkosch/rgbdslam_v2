...
//added to send all clouds since the last request. See GraphManager::sendAllClouds() for reference
void GraphManager::sendDiff(bool threaded){
	if(ParameterServer::instance()->get<bool>("concurrent_io") && threaded){
		QFuture<void> f1 = QtConcurrent::run(this, &GraphManager::sendDiffImpl);
...
void GraphManager::sendDiffImpl()
{
  ScopedTimer s(__FUNCTION__);
...
//check if we have sent the node before:
	if(node->timestamp_ < lastDiffTS_){
		ROS_INFO("Not sending frame because it has been sent already (TS of dropped node: %.2f)", node->timestamp_.toSec());
		continue;
	} 
	if(node-> timestamp_ > last || !last.isValid()){
		last = node->timestamp_;
	}
